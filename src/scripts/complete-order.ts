...
(async () => {
    try {
        ...
        const ordersToComplete = await orderRepository.findOrderToComplete(new Date().toISOString());
        console.log(`total orders to processed: ${ordersToComplete.length}`);

        for (let i = 0; i < ordersToComplete.length; i += 1) {
            const order = ordersToComplete[i];
            console.log(`current order ${i + 1}: ${order.id}`);

            await orderService.completeOrder(order.user_id, order.id, 'ADMIN');
            await notificationService.createNotification(order.user_id, NotificationMessage.SUCCESS, order.id);
        }
    } catch (err) {
        console.log(`error when completing order:`, err);
        process.exit(1);
    } finally {
        console.log('Done');
        process.exit(0);
